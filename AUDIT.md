...
- [ ] `static/robots.txt` has no explicit `Disallow` for `/api/*` routes (found: 2026-09-01)
...
- [ ] `src/routes/[slug]/+page.svelte:125` JSON-LD is injected via `{@html ...}` without escaping `</script>` inside the JSON payload — a post title/description containing the literal string `</script>` breaks out of the script tag and allows markup/script injection; `src/routes/monthly-summary/+page.svelte:54` already escapes this correctly (`&lt;\/script&gt;`), making it an inconsistently-applied fix rather than a one-off — audit and fix all `jsonLd` `{@html}` sinks under `src/routes` (found: 2026-09-01)
